...
2. **実装指示書 / レビュー依頼 / スモーク依頼**: `docs/wip/<機能>/` に置いて Cursor/Codex へ渡す(1タスク1フォルダ)。
   API+Web を跨ぐ機能でも**実装指示書は1枚に統合**する(ブランチ/PR はバックエンド/フロントで2本のまま。DR-017④)。
...
多段レビューでリードタイムが伸びた反省から、検証はプロセス側で軽くする。

- **API新機能は Pest の feature テストを実装の完了条件**に含める。docs ベースの curl スモークは
  **デプロイ時のコールドスタート/本番設定確認に限定**(毎機能では回さない)。
- **フロントは vitest で API 契約を検証**する(既存慣習=`vi.fn` fetch モック。MSW は未導入。
  過去表記の「MSW」は fetch モックと読み替える。DR-017補足 2026-07-18)。実機確認は**リリース前の1回**に絞る。
- **小差分(目安200行以下)はレビュー1周**で切り上げる(Codex 多段往復をかけない)。
- **レビューはCodexが行う。Fableはテストも差分レビューも自分でやらない**。Fableはレビュー依頼書を書いて
  Codex に委譲し、Codexの品質ゲート結果と合否レポートを読んでマージ判断する(2026-07-18 ユーザー指示)。
...
- Fableの役割: スペック書き・レビュー依頼書の作成・委譲の指揮(オーケストレーション)・合否判断・デザイン判断。
  **コードの精読/差分レビュー/テスト実行はしない**(Codexへ委譲し結果を読む)。
