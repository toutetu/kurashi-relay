...
更新日: 2026-07-20  
状態: **完了**（A0〜A8。恒久文書へ蒸留済み）  
対象リポジトリ: `toutetu/kurashi-relay`  
置き換えた方針: DR-030 の Inertia 中心ハイブリッド構成  
確定した方針: React Router + TanStack Query + Laravel JSON API の同一オリジン構成（DR-034 / DR-038）

> この文書は完了保管である。通常作業の正本は `docs/architecture.md` / `docs/development-plan.md` /
> `docs/api-contract.md` / `docs/design-decisions.md`(DR-034/035/038) を参照する。
...
```text
AGENTS.mdと docs/archive/phases/api-first-spa-migration/implementation-plan.md を読み、
...
`docs/archive/phases/api-first-spa-migration/` に完了済み文書を残さない。
...
| A3 | 完了 | `#62` | 2026-07-20 | 19 route・未認証動作を記録 | docs revert |
| A4 | 完了 | `#63` + Cloud env | 2026-07-20 | `FRONTEND_MODE=spa`・本番smoke OK | （A6以降 flag削除） |
| A5 | 完了 | なし（operation） | 2026-07-20 | `stability-a5.md`・blockingなし | mode=inertia（当時） |
| A6 | 完了 | `#69` | 2026-07-20 | Inertia削除・本番確認OK | A5 release / PR revert |
| A7 | 完了 | `#70` | 2026-07-20 | frontend削除・Render Auto-Deploy停止 | PR revert |
| A8 | 完了 | `docs/api-first-spa-complete` | 2026-07-20 | 恒久文書同期・WIP archive | docs revert |
...
docs/archive/phases/api-first-spa-migration/implementation-plan.md を全文読んでください。
