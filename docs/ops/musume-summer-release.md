...
> **2026-07-19追記**: シード復元と本番ブラウザでの書き込み・母画面反映は確認済みで、
> 試験データも空へ戻した。残る確認は `migrate:status` と、娘さんとの実端末での書き込み・反映確認である。
> **2026-07-20追記**: 個人データAPIは家族共有トークン保護へ戻った（DR-042）。以下の curl は
> `X-Family-Token` が必要（`/api/health` のみ不要）。トークン値はリポジトリへ書かない。
...
# Laravel Cloud の FAMILY_TOKEN と同じ値をローカル環境変数へ入れる（コミットしない）
TOKEN="${FAMILY_TOKEN:?FAMILY_TOKEN を設定してください}"

# 1. 声かけのテンプレートが復元されていること(シード確認・ここが0なら手順4が抜けている)
curl -s -H "X-Family-Token: $TOKEN" "$API/api/koekake/tasks" | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);const a=j.data??j.tasks??j;const arr=Array.isArray(a)?a:(a.tasks||[]);console.log('タスク件数:',arr.length)})"

# 2. メンバーが復元されていること(0や error ならシードが抜けている)
curl -s -H "X-Family-Token: $TOKEN" "$API/api/rewards/summary?member=child" | head -c 300
```
...
for M in child mother; do
  curl -s -H "X-Family-Token: $TOKEN" "$API/api/rewards/summary?member=$M"        # lifetime_count / coins / points
  curl -s -H "X-Family-Token: $TOKEN" "$API/api/rewards/collections?member=$M"    # ゾンビ図鑑の蓄積
done
curl -s -H "X-Family-Token: $TOKEN" "$API/api/musume/plan"                        # 娘の見通し
curl -s -H "X-Family-Token: $TOKEN" "$API/api/koekake/tasks"                      # 声かけの使用記録
```
