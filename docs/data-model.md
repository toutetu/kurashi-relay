...
| 娘が準備し、母が同時に支援 | `activity` | 母 | 娘 | 「一緒に」は completion status |
| 母が家事を実施 | `activity` | 母 | 母 | — |
| 母が娘の分を代行 | `activity` | 母 | 母 | 「代行」は completion status |
...
声かけ完了の `status`（`together` / `parent_done`）と `actor_member_id` で表す（DR-049）。
