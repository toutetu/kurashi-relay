...
実装手順の履歴は `docs/archive/phases/database-unification/implementation-plan.md`、設計判断は
...
flowchart TB
    ...
    subgraph CORE["出来事の中心"]
        AE["activity_events<br/>いつ・何が・誰が"]
    end
...
2. `activity_events`: いつ、何が、誰が関わったか（`actor_member_id` 必須）
...
### 7.1 `activity_events`（出来事の正本）

母のクイック記録、娘のおしごと、声かけ、支援、家事など、実際に起きたことを
「いつ・何が・誰が」という共通行で記録する。予定との対応・取消は該当するときだけ従属行を作る。
人物役割は従属表ではなく、出来事行の `recorded_by_member_id` / `actor_member_id` で持つ
（DR-048 / DR-049 / DR-050）。
...
- recorded_by_member_id FK -> family_members（アプリへ入力した人）
- actor_member_id FK -> family_members（行為者。必須）
...
- `recorded_by_member_id` と `actor_member_id` は同一人物とは限らない
  （母が娘の完了を入力する場合は母=`recorded_by`、娘=`actor`）。

行為者の記録ルール（DR-050）:

- `actor_member_id` には、実際にその行為を行った人物を記録する。
- 娘が行った場合は娘を、母が行った場合は母を `actor_member_id` とする。
- 「母が娘の代わりに行った」「代行した」という専用の状態・区分は設けない。
  母が行った活動は、単に「母がその活動を行った」実績として記録する。
- 娘を対象として作成された予定を母が行った場合も、実績イベントの行為者は母とする。
  この場合は、娘の `planned_activities` と母の `activity_events` を `plan_actual_links`
  で関連付けることで、「娘の予定だったが、実際に行ったのは母」という関係を表現する。
- したがって `parent_done` / `proxy` / `substitute` などの代行状態、および
  「一緒に」専用の `together` 状態は、実績の正本には保存しない。
- 母と娘がそれぞれ実施した場合は、行為者ごとの `activity_events` を別行で残す
  （母側にも実績を残したい運用に合わせる）。

代表例:

| 出来事 | `event_type` | recorded_by | actor | 補足 |
|---|---|---|---|---|
| 母が娘へ起床の声かけ | `prompt` | 母 | 母 | 対象は単一家庭では暗黙に娘 |
| 母が娘の持ち物準備を支援 | `support` | 母 | 母 | 同上 |
| 娘が持ち物を確認 | `activity` | （入力者） | 娘 | — |
| 母が家事を実施 | `activity` | 母 | 母 | — |
| 母が娘の予定活動を実施 | `activity` | 母 | 母 | 娘予定とは `plan_actual_links`。代行区分は持たない |
| 母と娘がそれぞれ実施 | `activity` ×2 | （各入力） | 各行為者 | 「一緒に」専用 status は持たない。母側も別イベント |

### 7.2 （廃止）`activity_event_participants`

DR-048により廃止。人物役割は `activity_events` の列へ統合した。

### 7.3 （廃止）`activity_event_outcomes`

DR-036により廃止。活動実績は `activity_events`（`event_type=activity`）の存在と
`actor_member_id` で表す。`partial` / `deferred` / `unknown` は活動実績へ持ち込まない。
先送りは `reminder_schedules` や `planned_activities.status`、予定時刻の変更で扱う。
`together` / `parent_done` など「一緒に」「代行」の専用区分は実績の正本には持たない（DR-050）。
予定との関係は必要なときだけ `plan_actual_links` で表す。

### 7.4 `activity_event_cancellations`（新規・取消履歴）

取消時だけ1行を追加し、元イベントと固有情報を削除しない。
...
- `task_records` は `activity_events`（`actor_member_id` 含む）、必要な結果・報酬行へバックフィルする。
- `completion_events` は `activity_events` へバックフィルする（結果行は作らない）。
- 既存 `prompt_events` は次節の固有情報を残し、各行に対応する `activity_events` を作る。
...
`activity_events` の人物列を正本とする。`prompt_events` は声かけにだけ必要な情報を1対1で保持する。
...
- 日時・入力者・冪等キーは `activity_events`、声をかけた人・対象者は `actor` / `target` 列、取消は
...
- id
- provider: `google`
- subject_role: `mother` / `child`（取込先の家族役割。DR-047）
...
- UNIQUE (`provider`, `external_calendar_id`)（未接続時は `external_calendar_id` を NULL のままにする）
- OAuthトークン・同期トークンは平文保存しない。
- 母用・むすめ用で別接続を持てる（同じGoogleアカウント内の別カレンダーでも可）。
  接続前プレースホルダは `subject_role` ごとに1件を再利用する。
...
| `daily_tasks.status` | 予定に対応する有効な `activity_events`（`event_type=activity`）の有無と actor / supporter / target | APIで導出。移行後に列削除 |
...
- API-first SPA移行後も通常画面は `/api/*` を利用する。現行の保護方式は DR-042
  （一時的な未認証公開の記録は DR-035 /
  `docs/archive/phases/api-first-spa-migration/access-contract-a3.md`）。
  実装指示の保管は `docs/archive/specs/family-token/`。

## 16. 実装順

実際の移行順、バックフィル、互換期間、検証条件は
`docs/archive/phases/database-unification/implementation-plan.md` を正とする。同計画は2026-07-19にDR-027〜DR-032へ同期済みである。
Gate 2 クローズは `docs/archive/phases/database-unification/gate2-close-record.md`（DR-041）。
家族共有トークンは DR-042 で完了。
今後同計画と本書が矛盾した場合は、最新のDRと本書を先に更新してから実装する。
Phase C/D1/Eのtarget schemaとSPA移行(A0〜A7)は完了済み（DR-034 / DR-038）。
次の中核は Product Phase 1 と必要に応じた Phase F とする。
