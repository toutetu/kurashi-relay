...
- target_member_id FK nullable -> family_members（声かけ・代行の対象）
- supporter_member_id FK nullable -> family_members（一緒にしたときの支援者）
...
- `ended_at` / `target_member_id` / `supporter_member_id` 以外の上記業務列は全て NOT NULL
- `event_type` CHECK
...
| 出来事 | `event_type` | actor | target | supporter |
|---|---|---|---|---|
| 母が娘へ起床の声かけ | `prompt` | 母 | 娘 | — |
| 母が娘の持ち物準備を支援 | `support` | 母 | 娘 | — |
| 娘が持ち物を確認 | `activity` | 娘 | — | — |
| 娘が準備し、母が同時に支援 | `activity` | 娘 | — | 母 |
| 母が家事を実施 | `activity` | 母 | — | — |
...
`supporter_member_id` / `actor`・`target` 列で表す。
