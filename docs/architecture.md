...
- 入力検証
- 認証・権限（家族共有トークン。`/api/health`以外。DR-042）
- DB保存
...
- 端末ごとのレスポンシブ再配置
- 家族共有トークン（あいことば）の端末内保存と401時の再入力
...
SPA移行中は一時的に未認証公開へ戻した（DR-035）が、DR-042で家族共有トークンを再接続した。
`/api/health` 以外は `X-Family-Token` 必須。詳細は `docs/api-contract.md`「現行アクセス契約」。
