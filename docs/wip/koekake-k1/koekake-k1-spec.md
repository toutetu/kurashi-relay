...
Schema::create('routine_templates', function (Blueprint $table) {
    $table->id();
    $table->string('activity_key', 16)->nullable()->index(); // docx設計書のACT-xxx。非ユニーク(同一活動が朝夜2枠等)。未対応はnull
...
- `activity_key` は docx設計書 §5 の活動マスタに対応があるものだけ設定。**対応が無いものは null のまま。
  新しいACTキーを勝手に発行しない**(キー発行は活動マスタ側=docx改訂の役割。docx§4.1)。
- `default_time` は**プレースホルダ**(実運用の時刻はユーザー実機確認で調整。シーダー修正のみで変更可)。
- `daily_limit` は docx の上限値をそのまま転記(K1では評価しない)。

| # | phase | sort | name | icon | activity_key | daily_limit | default_time | parent_prompt_label | child_label(箱) | quick_label(箱) |
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | morning | 1 | 起床 | ⏰ | null | null | 07:00 | 起きるように声をかけた | null | 起床の声かけ |
| 2 | morning | 2 | 朝食 | 🍞 | ACT-004 | 3 | 07:20 | ご飯を食べるように声をかけた | ご飯を食べた | 食事の声かけ |
| 3 | morning | 3 | 歯磨き | 🪥 | ACT-005 | 2 | 07:40 | 歯をみがくように声をかけた | 歯をみがいた | 歯みがきの声かけ |
| 4 | morning | 4 | 服薬 | 💊 | null | null | 07:45 | 薬を飲むように声をかけた | null | 服薬の声かけ |
| 5 | morning | 5 | 着替え | 👕 | ACT-003 | 1 | 07:50 | 着替えるように声をかけた | 着替えた | 着替えの声かけ |
| 6 | morning | 6 | 日焼け止め | 🧴 | null | null | 08:00 | 日焼け止めを塗るように声をかけた | null | 日焼け止めの声かけ |
| 7 | morning | 7 | 持ち物 | 🎒 | null | null | 08:05 | 持ち物を確認するように声をかけた | null | 持ち物確認の声かけ |
| 8 | morning | 8 | 出発 | 🚪 | null | null | 08:15 | 出発するように声をかけた | null | 出発の声かけ |
| 9 | evening | 1 | 帰宅確認 | 🏠 | null | null | null | 帰宅後にひと息つくよう声をかけた | null | 帰宅後の声かけ |
| 10 | evening | 2 | 今日の予定 | 📋 | null | null | null | 今日することを確認するように声をかけた | null | 予定確認の声かけ |
...
| 14 | evening | 6 | 明日の持ち物 | 🎒 | null | null | null | 明日の持ち物を確認するように声をかけた | null | 持ち物確認の声かけ |
...
| 19 | night | 5 | 服薬 | 💊 | null | null | 20:40 | 夜の薬を飲むように声をかけた | null | 服薬の声かけ |
...
| 22 | night | 8 | おやすみ | 🌙 | null | null | 21:45 | おやすみの挨拶をした | null | おやすみの声かけ |

マッピング注記(近似を含む。K2の活動マスタ統合時に見直す):

- 朝食と夕食は同じ ACT-004(docxの上限3=食事3回)を共有する。歯磨きも朝夜で ACT-005 を共有(上限2)。
- 「宿題」は docx の ACT-015(宿題をした)に対応させた(ACT-034 宿題確認は別活動として K2 で扱う)。
- 「明日の予定」は ACT-035(時間割確認)への近似マッピング。
- 「明日の持ち物」(夕方=確認)と「明日の準備」(夜=用意)のうち、docx§7.5 で夜に置かれた
  ACT-022 は夜側だけに対応させた。
...
1. **娘ポイント制**(1回1pt・上限・学習60分=お菓子1個100円換算)と既存おしごと
   (task_records の granted_point_value・ハロウィンラリー報酬)の関係。二重のポイント経済に
   なるため、統合するか分けるかを決めてから娘側実装に入る。
2. **K2娘用ホームのコンセプト**: 計画のD-01(見通し3項目中心)と docx F-01(できたこと記録中心)は
   画面の主役が異なる。K2モック作成前に方向を確認。
3. docx の時間帯区分(朝/帰宅後・夕方/夜/**いつでも**)の「いつでも」の扱い
   (phase は string なので追加自体は可能)。
4. ACT-023 は docx 本文に定義が無い欠番(受入条件に「表示されない」とだけ記載)。廃止キー扱いとし、
   割り当てない。
