...
- 通常は差分マイグレーションを使うが、Inertia移行前に限り、DR-031の条件を満たして管理下のDB refreshを選べる。
...
8. Inertia I0〜I7（`docs/wip/inertia-migration/implementation-plan.md`）
9. 支援者向けレポートMVP

Phase Aはフロントの401対応・ヘッダ付与を先に公開し、`FAMILY_TOKEN` 設定後にバックエンド保護を
有効化する。DBは統合後の箱を完成させたら、二重書込・新旧比較・安定観測を待たずInertia I0へ進む。
...
データ保持へ判断を変更した場合だけ使う代替経路であり、Gate 2やInertia I0の開始条件にしない。
...
**データ保持を選ぶ場合だけ実施する。DR-031のrefresh pathではInertia開始前に実施しない。**
...
**データ保持を選ぶ場合だけ実施する。refresh後はInertia側を新しい書込経路として接続する。**
...
- 現行レスポンス互換アダプターと旧APIの書込切替はInertia開始条件にしない
...
Gate 2通過後のPR単位はInertia移行計画を正とする。
...
DR-031・DR-032のrefresh pathではD2〜D5と本番安定観測を待たず、migration/seed確認後にInertia I0へ進む。
