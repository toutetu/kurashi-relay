...
その上で、Google Calendarから取り込んだ予定と実績を突き合わせられる構造にする。
...
- `activity_events(planned_activity_id)`: 1予定に複数の実績区間・中断・再開があり得るため。
...
1. `prompt_count` と `latest_prompt_at` を `prompt_events` の集計で返すクエリへ変更する。
2. 既存キャッシュ値と集計結果を比較するテストを追加する。
3. 本番で不一致がないことを確認する。
4. APIが集計値だけを使う状態になった後、キャッシュ列を差分ALTERで削除する。

推奨インデックス:

```sql
CREATE INDEX prompt_events_active_task_time
ON prompt_events (daily_task_id, prompted_at DESC)
WHERE cancelled_at IS NULL;
```

### 5.2 完了状態

1. `completion_events` の上書きを停止し、`activity_events` へ完了結果を追記する。
2. 現在状態は最新の非取消イベントから導出する。
...
- 取消は `cancelled_at` または取消イベントで表す。
- `recorded_by_member_id`、`source`、`recorded_at` を残す。
- APIのDELETEは物理DELETEではなく取消操作とする。

### 6.2 各領域

- 声かけ: 既存 `prompt_events.cancelled_at` を継続。
- おしごと実績: `activity_events` へ追記し、取消も保持。
- 完了状態: `updateOrCreate` を廃止し、訂正イベントを追記。
...
- `planned_activities`、`activity_events`、`reward_rules`、`reward_transactions` 作成
- `daily_tasks` を `planned_activities` へバックフィル
- `task_records` と `completion_events` を `activity_events` へバックフィル
- 既存ポイント・調整値を台帳へバックフィル
- 一時的に新旧へ同一トランザクションで書き込む
- 集計比較ジョブで日別件数・ポイント・コイン・完了状態を比較
- 読取を新テーブルへ切替
- 安定後に旧書込を停止

完了条件:

- おしごとと声かけ画面の完了が同じ `activity_events` から取得できる。
- 声かけ履歴は混在せず `prompt_events` のまま参照できる。
- 既存集計と新集計が一致する。
...
- `plan_actual_links`、`schedule_impacts` 作成
- まず時間重なり+活動キーで自動候補を出す
- 自動一致は確定扱いせず、`matched_by` と `confidence` を保持
- 手動修正可能にする
- 待機・回復・支援を実績kindとして関連付ける

完了条件:

- 予定なし実績、実績なし予定、遅延、中断、短縮、回復への変更を表現できる。
...
5. `feat/activity-spine-api`
6. `feat/activity-spine-web`
7. `feat/plan-history-api`
8. `feat/plan-history-web`
9. `refactor/derived-values-api`
10. `feat/google-calendar-api`
11. `feat/schedule-comparison-api`
12. `feat/schedule-comparison-web`
