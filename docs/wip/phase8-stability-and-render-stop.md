...
| 9 | スマートフォンからの利用 | 実機 | 375px で `/oshigoto` 正常・完了/取消が保存される | 🟡 表示合格、保存契約修正の本番再確認待ち |
...
- `/oshigoto` の通常保存はDBへ成功したが、APIが報酬なしの `revealed_reward` を省略するため、
  フロントが「おしごとの保存結果が正しくありません」と誤表示する不具合を検出した。
  `codex/fix-koekake-scheduled-at-timezone` の `938c849` で `revealed_reward: null` を常に返す修正と
  回帰テストを追加済み。本番反映後に#9を再確認する。
...
- フロントは74テスト、typecheck、lint、buildに合格した。通信断→「あとで保存します」→online再送は
