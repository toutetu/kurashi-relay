...
class MusumePlanTest extends TestCase
{
    ...
    public function test_musume_summary_returns_plan_content_when_exists(): void
    {
        ...
        $this->getJson('/api/koekake/musume-summary?date=2026-07-18')
            ...
            ->assertJsonPath('summary.today_items', [])
            ->assertJsonPath('summary.bedtime', null)
            ->assertJson(fn (AssertableJson $json) => $json
                ->has('summary.decided_with.today')
                ->has('summary.decided_with.today_item')
                ->where('summary.decided_with.today_item', null)
                ->has('summary.decided_with.bedtime')
                ->where('summary.decided_with.bedtime', null)
